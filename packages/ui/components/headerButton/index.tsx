...
export function HeaderButton({ href, className }: HeaderButtonProps) {
  return (
    <Link
      href={href}
      className={clsx(
        'flex justify-center items-center bg-transparent mobile:w-5 mobile:h-5 tablet:w-7 tablet:h-7 desktop:w-7 desktop:h-7',
        className,
      )}
    >
      <svg
...
export function HeaderTitle({ title, children }: HeaderTitleProps) {
  return (
    <div className='flex mobile:gap-[0.375rem] tablet:gap-2 desktop:gap-2'>
