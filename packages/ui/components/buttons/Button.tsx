...
const ButtonMediumBaseStyle =
  'box-border inline-flex justify-center items-center border-[0.0938rem] rounded-[2.5rem] text-center active:shadow-none disabled:shadow-none disabled:cursor-not-allowed h-10 px-6 text-sm font-normal leading-tight';

/**
 *
 * @author `송용수`
 *
 * @desc Button medium 사이즈 UI 컴포넌트. gap 속성 제거. (tablet 사이즈 고정)
 *
 * @param type
 * — html `<button>` 태그의 type 속성에 들어가는 값.
 * - `필수 파라미터` — `X`
...
export function ButtonMedium({
  ...
  return (
    <button
      type={type}
      className={clsx(ButtonMediumBaseStyle, ButtonModeStyle[mode], className)}
      disabled={isDisabled}
      onClick={onClick}
    >
      {children}
